...
## Live sequential QR send + filter-select (2026-07-27)
- Added a per-(org,event) WebSocket room hub (src/socket/hub.ts) + a query-token stream route (src/routes/stream.ts) mounted at /api/stream, outside the Bearer-header authed router. Every teammate on the event participants tab subscribes via useEventStream and sees the same live feed.
- POST /:id/participants/send sends ONE QR at a time (config.sendDelayMs, default 700ms) to dodge Gmail burst limits, broadcasting start/item(sending|sent|failed)/done. Failures are aggregated into one failureReportEmail (table: name, email, reason, source, custom fields) sent to owner + all collaborators.
- Frontend: SendProgress modal (progress bar + live per-recipient feed, newest first), filter bar (QR sent/unsent, checked-in/pending, source, per-custom-field contains) + "Select all N matching" so any state combo can be bulk-selected and sent.
- Could improve next: run the send as a detached background job (so a very large list survives the initiator navigating away / request timeouts) and persist a send-run log; and lift the socket to the event page so teammates on other tabs also see progress.
